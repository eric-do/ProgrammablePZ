...
const getRides = async (req, res, next) => {
  try {
    ...
    if (user) {
      res.locals.data = await RideModel.getRidesCreatedByUser(user, type, timeInSeconds, limit, page);
    } else {
      res.locals.data = await RideModel.getRides(type, timeInSeconds, limit, page);
    }
    res.status(200);
    next();
  } catch (err) {
