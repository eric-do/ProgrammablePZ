...
-- ADD FUNCTION TO REFRESH VIEW AS TRIGGER
DROP FUNCTION IF EXISTS refresh_admin_rides_tg() CASCADE;
CREATE OR REPLACE FUNCTION refresh_admin_rides_tg()
...
ON rides
FOR EACH STATEMENT EXECUTE PROCEDURE refresh_admin_rides_tg();

-- SET FUNCTION TO EXCECUTE WHEN A RIDE IS RATED
CREATE OR REPLACE TRIGGER tg_refresh_admin_rides AFTER INSERT OR UPDATE OR DELETE
ON ride_ratings
FOR EACH STATEMENT EXECUTE PROCEDURE refresh_admin_rides_tg();

-- SET FUNCTION TO EXCECUTE WHEN A RIDE IS TAKEN
CREATE OR REPLACE TRIGGER tg_refresh_admin_rides AFTER INSERT OR UPDATE OR DELETE
ON user_rides
FOR EACH STATEMENT EXECUTE PROCEDURE refresh_admin_rides_tg();